...
describe('EventBus', () => {
  ...
  it('should emit events', () => {
    ...
    eventBus.announce(createEvent(SESSION.STARTED, 43));
    eventBus.announce(createEvent(SESSION.STOPPED));
  ...
  describe('event subscriptions', () => {
    ...
    it('should subscribe to "most recent" event only 1x', () => {
      ...
      // Fire event BEFORE subscription
      eventBus.announce(createEvent(userChanged, 'thomas burleson'));

      const unsubscribe = eventBus.on<number>(userChanged, trackValue);
      expect(recent).toBe('thomas burleson');

      eventBus.announce(createEvent(userChanged, 'wes grimes'));
      expect(recent).toBe('wes grimes');

      unsubscribe();

      eventBus.announce(createEvent(userChanged, 'jon rista'));
      expect(recent).toBe('wes grimes');
    });
    ...
    it('should unsubscribe from future events', () => {
      ...
      unsubscribe();
      eventBus.announce(createEvent(SESSION.STARTED, 100));
  ...
  describe('event stream observables', () => {
    ...
    it('should stream subscribed events', () => {
      ...
      eventBus.announce(createEvent(SESSION.UNKNOWN, 100));
      expect(value).toBe(43);
