...
import { Observable, OperatorFunction, Subject, of, pipe } from 'rxjs';
 ...
 */
const delayWhen = (val: number | undefined) =>
  val ? pipe(delay(val)) : pipe();
...
export class EventBus {
  ...
  /**
   * Easily emit an event type with optional data
   */
  emit<T>(event: string, data?: T) {
    this.announce({ type: event, data });
  }
  ...
  onMany(collection: EventRegistrations): Unsubscribe {
    ...
    const connections = eventKeys.map(
      (key) => collection[key] && this.on(key, collection[key]),
    );

    return () => {
      connections.map((teardown) => teardown && teardown());
    };
  }
  ...
  observableFor<T>(event: string): Observable<T> {
    return this.emitter.pipe(
      startWith(this.cache[event] || ({ type: 'initialized' } as EmitEvent<T>)),
      takeUntil(this.destroy$),
      filter((e) => (e as EmitEvent<T>)?.type === event),
      map((e) => (e as EmitEvent<T>).data),
      delayWhen(this.options.delayNotify) as OperatorFunction<unknown, unknown>,
    ) as Observable<T>;
  }
