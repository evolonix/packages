...
describe('Event Reporting', () => {
  ...
  describe('reportBefore()', () => {
    it('should NOT pass target emission to notifyBefore callback', async () => {
      ...
      const notifyBefore = vitest.fn();
      const requestEmission = vitest.fn();
    ...
    it('should trigger the notifyBefore BEFORE the stream value emits', () => {
      let callbackTime = 0;
  ...
  describe('reportAfter()', () => {
    ...
    it('should emit notifyCallback response if defined', async () => {
      const startTime = getTime().valueOf();
      const notifyCallback = vitest.fn();
    ...
    it('should emit stream value if notifyCallback response is undefined', async () => {
      const startTime = getTime().valueOf();
      const notifyCallback = vitest.fn();
      const notifyResponse = await lastValueFrom(
        reportAfter(of(startTime), notifyCallback),
      );

      expect(notifyResponse).toBe(startTime);
    });

    it('should call the notifyCallback AFTER the stream value emits', () => {
      let notifyTime = 0;
      const startTime = getTime().valueOf();
